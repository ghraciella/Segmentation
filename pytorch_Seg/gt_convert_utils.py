...
def visualizeMask(mask, IdConverter, rgbimage = None, flip = False):
    ...
    if not flip:
        for classId in range(classes):
            outputimage[mask == classId] = cityscapes_label2color[IdConverter[classId]] #tuple(reversed(cityscapes_label2color[IdConverter[classId]]))
    else:
        for classId in range(classes):
...
if __name__ == "__main__":
    ...
    classes = range(34) #[7] #list(range(33))# labelsWithTrainID
    color_converter = get_color_converter(classes)
    class_converter = ClassConverter(classes)
    newmask = class_converter(testGT)
    visualizeMask(newmask, color_converter, len(classes), testImageRGB)
